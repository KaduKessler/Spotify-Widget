...
function validateConfig(config: ReturnType<typeof buildConfig>) {
  ...
  if (
    ...
    !config.ENABLE_GITHUB_AUTH &&
    !config.ENABLE_NONE_AUTH
  ) {
    errors.push(
      'At least one auth method must be enabled (ENABLE_PASSWORD_AUTH, ENABLE_GITHUB_AUTH, or ENABLE_NONE_AUTH)',
    )
  }
  ...
  if (config.ENABLE_PASSWORD_AUTH) {
    if (!config.ADMIN_USERNAME || config.ADMIN_USERNAME === 'admin') {
      errors.push(
        'ADMIN_USERNAME must be set to a secure value when password auth is enabled',
      )
    }
    if (
      ...
      errors.push(
        'ADMIN_PASSWORD must be set to a strong password (min 8 chars) when password auth is enabled',
      )
    }
  }
  ...
  if (config.ENABLE_GITHUB_AUTH) {
    if (
      ...
      config.GITHUB_CLIENT_ID === 'your_github_client_id'
    ) {
      errors.push('GITHUB_CLIENT_ID must be set when GitHub auth is enabled')
    }
    if (
      !config.GITHUB_CLIENT_SECRET ||
      config.GITHUB_CLIENT_SECRET === 'your_github_client_secret'
    ) {
      errors.push(
        'GITHUB_CLIENT_SECRET must be set when GitHub auth is enabled',
      )
    }
  }
  ...
  if (
    ...
    (!config.SESSION_SECRET ||
      config.SESSION_SECRET === 'devSessionSecret' ||
      config.SESSION_SECRET.length < 32)
  ) {
    errors.push(
      'SESSION_SECRET must be set to a strong random value (min 32 chars) in production. Generate with: openssl rand -hex 32',
    )
  }
...
function buildConfig() {
  ...
  const ENABLE_PASSWORD_AUTH = parseBoolean(
  ...
  if (
    ...
    console.warn(
      '[Config] AUTH_PROVIDER is deprecated. Use ENABLE_PASSWORD_AUTH, ENABLE_GITHUB_AUTH instead.',
    )
  ...
  return {
    ...
    REGISTRATION_POLICY: (process.env.REGISTRATION_POLICY || 'open') as
      ...
      | 'invite_only'
      | 'closed',
    GITHUB_WHITELIST: process.env.GITHUB_WHITELIST
      ? process.env.GITHUB_WHITELIST.split(',')
          .map((s) => s.trim())
          .filter(Boolean)
      : ([] as string[]),
    ALLOW_PASSWORD_SIGNUP: parseBoolean(
      process.env.ALLOW_PASSWORD_SIGNUP,
      true,
    ),
...
export function loadConfig() {
  ...
  if (config.NODE_ENV === 'production') {
    ...
    if (config.ENABLE_PASSWORD_AUTH && config.ADMIN_PASSWORD === 'admin') {
      console.warn(
        '⚠️  Warning: Using default ADMIN_PASSWORD. Change it in production!',
      )
    }
    ...
    if (!validPolicies.includes(config.REGISTRATION_POLICY)) {
      console.warn(
        '⚠️  Warning: Invalid REGISTRATION_POLICY. Using "open" as default.',
      )
    ...
    if (
      ...
      console.warn(
        '⚠️  Warning: REGISTRATION_POLICY=github_whitelist but GITHUB_WHITELIST is empty.',
      )
    }
  }
